...
# =====================
# Day48 Geohash + Timestamp Demand
# =====================
...
train["day48_pair_demand"] = (
    train.set_index(
        ["geohash", "timestamp"]
    ).index.map(pair_mean)
)

test["day48_pair_demand"] = (
    test.set_index(
        ["geohash", "timestamp"]
    ).index.map(pair_mean)
)
...
prefix4_mean = (
    train.groupby("geohash_prefix_4")["demand"]
    .mean()
)

prefix5_mean = (
    train.groupby("geohash_prefix_5")["demand"]
    .mean()
)

train["prefix4_mean_demand"] = (
    train["geohash_prefix_4"]
    .map(prefix4_mean)
)

test["prefix4_mean_demand"] = (
    test["geohash_prefix_4"]
    .map(prefix4_mean)
)

train["prefix5_mean_demand"] = (
    train["geohash_prefix_5"]
    .map(prefix5_mean)
)

test["prefix5_mean_demand"] = (
    test["geohash_prefix_5"]
    .map(prefix5_mean)
)
...
print("\nSubmission saved:")
print("submissions/submission_v7.csv")
